...
pub fn setup_only_chinese_fonts(ctx: &Context) -> Result<(), FontError> {
    let mut fonts = FontDefinitions::default();

    // Load Chinese font data
    let chinese_font_data = load_chinese_font()?;
    ...
    // Keep default fonts first, then add Chinese font
    // This ensures other languages use default fonts, Chinese uses Chinese font
    fonts.families.entry(FontFamily::Proportional).or_default()
